...
class TestSyntacticFeatureExtractor(unittest.TestCase):
    ...
    def test_basic_feature_extraction(self):
        ...
        tokens = [
            ...
            {'word': 'killed', 'pos': 'VBD', 'index': 1, 'lemma': 'kill'},
            {'word': 'civilians', 'pos': 'NNS', 'index': 2, 'lemma': 'civilian'}
        ]
        dependencies = [
    ...
    def test_violence_verb_detection(self):
        ...
        tokens = [
            {'word': 'People', 'pos': 'NNS', 'index': 0, 'lemma': 'people'},
            {'word': 'walked', 'pos': 'VBD', 'index': 1, 'lemma': 'walk'},
            {'word': 'home', 'pos': 'NN', 'index': 2, 'lemma': 'home'}
        ]
    ...
    def test_empty_input(self):
        ...
        self.assertFalse(features['has_violence_verb'])
        # When dependencies are empty, these features may not be present
        if 'has_nsubj' in features:
            self.assertFalse(features['has_nsubj'])
        if 'has_dobj' in features:
            self.assertFalse(features['has_dobj'])
        if 'has_agent_patient' in features:
            self.assertFalse(features['has_agent_patient'])
